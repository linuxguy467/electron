...
ipcMainInternal.handle('ELECTRON_BROWSER_CLIPBOARD', function (event, method, ...args) {
  if (!allowedClipboardMethods.has(method)) {
    throw new Error(`Invalid method: ${method}`);
  }

  return new Promise((resolve) => {
    electron.clipboard[method](...typeUtils.deserialize(args)).then((result) => {
      resolve(typeUtils.serialize(result));
    });
  });
});
